...
const LeadDistribution = () => {
  ...
  const handleTransfer = async (leadId: string, salespersonId: string) => {
    try {
      await supabase.from("leads").update({ assigned_to: salespersonId }).eq("id", leadId);
  ...
  const getLeadDuration = (createdAt: string) => {
    ...
    if (seconds < 60) return { label: `${seconds} sec`, className: "bg-green-500 text-white" };
    if (minutes < 60) return { label: `${minutes} min`, className: "bg-green-500 text-white" };
    if (hours < 24) return { label: `${hours} hr`, className: "bg-green-500 text-white" };
    if (days < 30) return { label: `${days} day`, className: "bg-green-500 text-white" };
    if (months <= 3) return { label: `${months} month`, className: "bg-yellow-400 text-black" };
              ...
              <Table>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={9} className="text-center py-6 text-muted-foreground">No leads found</TableCell>
                    </TableRow>
                  ) : (
                    leads.map((lead, index) => {
                      ...
                      return (
                        <TableRow key={lead.id}>
                          ...
                          <TableCell><Badge className={stageBadgeColor(lead.stage)}>{lead.stage || "Lead"}</Badge></TableCell>
                          <TableCell><Badge className={duration.className}>{duration.label}</Badge></TableCell>
                          <TableCell>
                            {canManageLeads ? (
                              <Select onValueChange={(v) => handleTransfer(lead.id, v)}>
                                <SelectTrigger>
                                  <SelectValue placeholder={lead.profiles?.full_name || "Assign..."} />
                                </SelectTrigger>
                                <SelectContent>
                                  {salespeople.filter(p => p.id !== lead.assigned_to).map(person => (
                          ...
                          <TableCell className="text-right space-x-2">
                            <Button size="sm" variant="outline" onClick={() => navigate(`/leads/${lead.id}`)}><History className="h-4 w-4 mr-1" /> History</Button>
                            {canManageLeads && <Button size="sm" onClick={() => navigate(`/leads/new?edit=${lead.id}`)}><Edit className="h-4 w-4" /></Button>}
                            {userRole === "admin" && <Button size="sm" variant="destructive" onClick={() => handleDelete(lead.id)}><Trash2 className="h-4 w-4" /></Button>}
                          </TableCell>
                        </TableRow>
                      );
                    })
